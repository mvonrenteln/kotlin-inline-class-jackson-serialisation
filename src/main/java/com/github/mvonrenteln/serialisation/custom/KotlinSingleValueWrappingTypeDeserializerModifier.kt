 ...
 */
internal class KotlinSingleValueWrappingTypeDeserializerModifier : BeanDeserializerModifier() {
    /*
	 * (non-Javadoc)
    ...
    override fun modifyDeserializer(
        ...
        val type = descriptor.beanClass.kotlin
        if (type.hasAnnotation<ValueObject>() && type.declaredMemberProperties.size == 1) {
            val parameterType = descriptor.findProperties()[0].primaryType
            return ConstructorDeserializer(type, parameterType)
        }
...
private class ConstructorDeserializer(val kClass: KClass<out Any>, val parameterType: JavaType) : StdDeserializer<Any>(
    Any::class.java
) {
